...
export default function LoginPage() {
  ...
  // Segundo paso, solo si el usuario tiene MFA activado.
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      const message = err instanceof ApiRequestError ? err.message : 'No se pudo iniciar sesión';
      setError(message);
    } finally {
      setLoading(false);
    }
      ...
      <Paper sx={{ p: 5, width: '100%', maxWidth: 400, borderRadius: 3 }}>
        <Box sx={{ display: 'flex', justifyContent: 'center', mb: 3 }}>
          <Box
            ...
            alt="Expalsa"
            sx={{ height: 80, width: 'auto', maxWidth: '100%', objectFit: 'contain' }}
          />
        </Box>
        ...
        {!mfaChallengeToken ? (
          <>
            ...
            <Box component="form" onSubmit={handleSubmit} sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              ...
              <Link component={RouterLink} to="/forgot-password" variant="body2" sx={{ textAlign: 'center', mt: 1 }}>
                Olvidé mi contraseña
              </Link>
            </Box>

            <Typography variant="caption" sx={{ display: 'block', textAlign: 'center', mt: 3, color: 'text.secondary' }}>
              Usuarios de prueba (password: <code>password123</code>):
              <br />
              admin@demo.com · limitedadmin@demo.com · technician@demo.com
              <br />
              limitedtechnician@demo.com · requester@demo.com
            </Typography>
          </>
        ) : (
          <>
